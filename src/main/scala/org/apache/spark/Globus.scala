...
def main(args: Array[String]): Unit = {
	...
	// DBs loading:
	val homology_gene2ac = sc.broadcast(loadRDD("./resources/Homology/modelGeneID2SwissAC.txt").collect)
	val homology_ac2ec = sc.broadcast(loadRDD("./resources/Homology/modelUniprot2EC.txt").collect)

	val orthologs_ec = sc.broadcast(loadRDD("./resources/Orthology/geneID2othlogsECs.map").collect)

	val pg = sc.broadcast(loadContextFile("./resources/Context/Ecoli/zPG_stripped.txt").collect)
	val cx = sc.broadcast(loadContextFile("./resources/Context/Ecoli/zCX_stripped.txt").collect)
	val dc = sc.broadcast(loadContextFile("./resources/Context/Ecoli/zDC_stripped.txt").collect)

	val modelPG = sc.broadcast(loadContextFile("./resources/Context/Model/zPG_stripped.txt").collect)
	val modelCX = sc.broadcast(loadContextFile("./resources/Context/Model/zCX_stripped.txt").collect)
	val modelDC = sc.broadcast(loadContextFile("Context/Model/zDC_stripped.txt").collect)
	val modelNeighbors = sc.broadcast(loadEdges("./resources/Context/modelGeneNeighbors.txt").collect)

	val ec_coocurances = sc.broadcast(loadActivties("./resources/EC/newEC_coocurance.txt").collect)
